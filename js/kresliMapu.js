...
import { zlidstiCislo, vypisOrganizace, zjistiRokScitani, formatNumber, celkemSkautu, desetinnaCarka } from './helperFunctions';
              ...
              events: {
                  click: function() {
                      ...
                      if (vybranaData[0].popm) {
                        informace.innerHTML += `Benjamínci, vlčata a skauti tvoří <strong>${(skautido14/vybranaData[0].popm*100).toFixed(2)} % populace chlapců </strong> do 14 let.</br>`    
                      }
                      if (vybranaData[0].popm) {
                        informace.innerHTML += `Benjamínci holky, světlušky a skautky tvoří <strong>${(skautkydo14/vybranaData[0].popf*100).toFixed(2)} % populace dívek </strong> do 14 let.`    
                      }
                    }
              },
          }
